...
def lock(aims, mouse, top_x, top_y, len_x, len_y, args, pidx, pidy):
    ...
    aims_copy = [x for x in aims_copy
                 if x[0] in args.lock_choice and
                 (len_x * float(x[1]) + top_x - mouse_pos_x) ** 2 + (len_y * float(x[2]) + top_y - mouse_pos_y) ** 2 < detect_arange]
    ...
    if len(aims_copy):
        ...
        if dist_list:
            ...
            move.mouse_xy(round(pid_movex), round(pid_movey))
